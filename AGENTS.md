...
- 🚦 **Smoke/load тесты** (`#[Group('smoke')]`: `AuctionBidPerfSmokeTest`, `AuctionBidLoadSmokeTest`) **исключены из параллельного прогона** — это нагрузочные замеры (p95, bids/sec, реальные COMMIT через `SkipDatabaseRollback`), параллельно с остальными тестами они бессмысленны/нестабильны (конкуренция за CPU/БД/Redis). Запуск отдельно и строго последовательно, один за другим: `composer test:smoke` (`test:prepare` + `phpunit --configuration phpunit.smoke.xml --group=smoke`, один процесс; конфиг `phpunit.smoke.xml` — копия `phpunit.dist.xml` без QueryGuard Extension: нагрузочные замеры не подлежат query-правилам, DAMA и Foundry оставлены). Полный прогон «параллельно → потом smoke» — `composer test:parallel:full`; в CI так и настроено. Новый нагрузочный тест — тоже `#[Group('smoke')]`.
...
- State machines (tender 16 статусов/38 переходов, lot, auction, contract) — **symfony/workflow** (config/packages/workflow.yaml + WorkflowInterface), не самописные переходы. Статус лота двигают тендер (публикация/старт приёма заявок/отмена) и аукцион лота — карта переходов и обоснование в `domain/tender-state-machine.md`, раздел 3a.
