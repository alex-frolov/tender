...
/**
 * Voter права на экспорт данных (UC-31, F-A7, FR-1.5.10/1.5.15): делегирует
 * проверку PermissionCheckerInterface::can('exports.export'). Проверяем:
 * supports, отказ для не-App-пользователя/неизвестного атрибута и результат = can().
 */
final class ExportVoterTest extends TestCase
{
    private ExportVoter $voter;
    ...
    protected function setUp(): void
    {
        ...
        $this->voter = new ExportVoter($this->permissions);
    }
    ...
    public function testGrantedWhenPermissionAllows(): void
    {
        ...
        self::assertSame(
            ...
            $this->vote(ExportVoter::EXPORT, $this->token($user)),
        );
    }
    ...
    public function testDeniedForAnonymous(): void
    {
        self::assertSame(
            VoterInterface::ACCESS_DENIED,
            $this->vote(ExportVoter::EXPORT, $this->token(null)),
        );
    }
