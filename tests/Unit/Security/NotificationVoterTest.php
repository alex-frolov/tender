...
/**
 * Voter прав на подписки уведомлений (FR-1.6.3): делегирует
 * проверку PermissionCheckerInterface::can('notifications.subscribe'). Проверяем:
...
final class NotificationVoterTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->voter = new NotificationVoter($this->permissions);
    }
    ...
    public function testGrantedWhenPermissionAllows(): void
    {
        ...
        self::assertSame(
            ...
            $this->vote(NotificationVoter::SUBSCRIBE, $this->token($user)),
        );
    }
    ...
    public function testDeniedForAnonymous(): void
    {
        self::assertSame(
            VoterInterface::ACCESS_DENIED,
            $this->vote(NotificationVoter::SUBSCRIBE, $this->token(null)),
        );
    }
