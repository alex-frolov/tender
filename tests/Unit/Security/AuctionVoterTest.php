...
/**
 * Voter прав на аукцион (FR-1.3.5, FR-1.5.10/1.5.15): делегирует проверку
 ...
 */
final class AuctionVoterTest extends TestCase
{
    private AuctionVoter $voter;
    ...
    protected function setUp(): void
    {
        ...
        $this->voter = new AuctionVoter($this->permissions);
    }
    ...
    public function testSupportsOnlyKnownAttributesWithNullSubject(): void
    {
        ...
        self::assertSame(VoterInterface::ACCESS_GRANTED, $this->vote(AuctionVoter::CREATE, $token));
        self::assertSame(VoterInterface::ACCESS_GRANTED, $this->vote(AuctionVoter::UPDATE, $token));
        self::assertSame(VoterInterface::ACCESS_GRANTED, $this->vote(AuctionVoter::SCHEDULE, $token));
        self::assertSame(VoterInterface::ACCESS_GRANTED, $this->vote(AuctionVoter::CANCEL, $token));
        self::assertSame(VoterInterface::ACCESS_GRANTED, $this->vote(AuctionVoter::FINISH, $token));
        self::assertSame(VoterInterface::ACCESS_GRANTED, $this->vote(AuctionVoter::CHOOSE_WINNER, $token));
        self::assertSame(VoterInterface::ACCESS_GRANTED, $this->vote(AuctionVoter::BID, $token));
    }
    ...
    public function testNonAppUserTokenDenied(): void
    {
        ...
        self::assertSame(VoterInterface::ACCESS_DENIED, $this->vote(AuctionVoter::FINISH, $token));
    }
    ...
    public function testDelegatesToPermissionCheck(): void
    {
        ...
        $permissions->expects(self::once())
            ->method('can')
            ->with($admin, 'auction.choose_winner')
            ->willReturn(true);
        $voter = new AuctionVoter($permissions);

        self::assertSame(VoterInterface::ACCESS_GRANTED, $voter->vote($this->token($admin), null, [AuctionVoter::CHOOSE_WINNER]));
    }
    ...
    public function testDeniedWhenPermissionFails(): void
    {
        ...
        $permissions->expects(self::once())
            ->method('can')
            ->with($agent, 'auction.control')
            ->willReturn(false);
        $voter = new AuctionVoter($permissions);

        self::assertSame(VoterInterface::ACCESS_DENIED, $voter->vote($this->token($agent), null, [AuctionVoter::FINISH]));
    }
}
