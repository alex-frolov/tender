...
/**
 * Форма изменения лота (FR-1.1.7, PATCH /tenders/{tenderId}/lots/{lotId}).
 * Все поля опциональны (null = не менять); пустая строка/[] = очистить.
 * Правка допустима до окончания приёма заявок на тендер.
 * Имена полей — snake_case (как в openapi LotUpdate).
 *
 * @extends AbstractType<LotUpdateInput>
 */
final class LotUpdateType extends AbstractType
{
    ...
    public function configureOptions(OptionsResolver $resolver): void
    {
        $resolver->setDefaults([
            ...
            'data_class' => LotUpdateInput::class,
        ]);
    }
}
