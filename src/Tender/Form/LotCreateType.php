...
/**
 * Форма добавления лота в существующий тендер (FR-1.1.7, POST /tenders/{tenderId}/lots).
 * Поля — как LotType при создании тендера (title, price_net_minor обязательны;
 * vat_rate/price_basis наследуются от тендера, если не заданы).
 * Имена полей — snake_case (как в openapi LotCreate); маппинг на camelCase
 * свойства DTO — через property_path.
 *
 * @extends AbstractType<LotCreateInput>
 */
final class LotCreateType extends AbstractType
{
    ...
    public function configureOptions(OptionsResolver $resolver): void
    {
        $resolver->setDefaults([
            ...
            'data_class' => LotCreateInput::class,
        ]);
    }
}
