 ...
 *     * OWNER_ONLY (подготовка торгов) — никому, кроме заказчика: это его
 *       внутренние решения;
 ...
 *     * OWNER_AND_WINNER (пауза, выбор победителя, исполнение, служебные
 *       исходы) — только компании-исполнителю: победителю лота
 *       (BidWinnerQuery). Пока победителя нет — фактически только заказчику.
 *
...
final class AuctionStreamVoter extends Voter
{
    ...
    protected function voteOnAttribute(string $attribute, $subject, TokenInterface $token, ?Vote $vote = null): bool
    {
        ...
        return match ($subject->getStatus()->visibilityLevel()) {
            // Подготовка торгов — только заказчику: наружу этой стадии нет,
            // даже допущенному участнику.
     ...
     *
     * Проверка здесь строгая (isAdmitted, не isAdmittedToTrade): у тендера
     * без заявок на участие следа участия не остаётся, и «участник посреди
     * торгов» неотличим от постороннего — такому зрителю остаётся общая
     * видимость тендера.
     */
